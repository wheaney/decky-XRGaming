...
* **In Virtual Display mode**:
  ...
  * Advanced Settings / **Bind recenter button**. Bind a button on any connected controller (including the Deck's built-in controller) to trigger a recenter, so you can re-center the display mid-game without opening the Quick Access Menu. Click the button, then press the controller button you want to use; click again to rebind, or use **Clear recenter button binding** to remove it.
