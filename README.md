...
* **In Virtual Display mode**:
  ...
  * Advanced Settings / **Recenter controller combo**. Enabling this lets you trigger a recenter mid-game without opening the Quick Access Menu, by holding L4 + R4 (the back grip buttons) together. No Steam Input mapping needed — it listens for the combo directly via Steam's own controller input API. Under the hood it runs the bundled `recenter.sh`, a standalone one-line script (`echo "recenter_screen=true" > /dev/shm/xr_driver_control`) that you can also bind to any other key/button-binding tool if you'd rather not use the plugin's toggle.
