...
* **In Virtual Display mode**:
  ...
  * Advanced Settings / **Ctrl+Alt+R recenter shortcut**. Enabling this lets you trigger a recenter mid-game without opening the Quick Access Menu. Map a controller button to the `Ctrl+Alt+R` keyboard shortcut in Steam's own controller layout settings (per-game or globally), and pressing that button will recenter the display.
